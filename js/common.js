class SimuladorPlaylists {
    ...
    eliminarPlaylist(nombre) {
        const index = this.playlists.findIndex(pl => pl.nombre === nombre);
        if (index !== -1) {
            this.playlists.splice(index, 1);
            this.guardarEnStorage();
            console.log("✅ Playlist eliminada:", nombre);
        } else {
    ...
    cargarDesdeStorage() {
        ...
        return playlists.map(pl => {
            ...
            playlist.canciones = pl.canciones ? pl.canciones.map(c => new Cancion(c.nombre, c.duracion)) : [];
            playlist.duracionTotal = pl.duracionTotal || 0;
            return playlist;
        });
    }
}
